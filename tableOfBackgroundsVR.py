...
for regionID in range(1, 5):
  for fname in glob.glob("VR1L/VR1L{0:d}Cuts/*.json".format(regionID))+glob.glob("VR0L/VR0L{0:d}Cuts/*.json".format(regionID)):
    did = os.path.basename(fname).split('.')[0]

    # figure out if we're using 1L or 0L
    region='VR1L'
    if 'VR0L' in os.path.basename(os.path.dirname(fname)): region='VR0L'
...
def getValues(group, groups):
  return [group] + [groups[group][region][i][index] for region in ['VR0L', 'VR1L'] for i in range(1,5)]
...
for index, typeBkgd in zip(count_types, ['raw', 'weighted', 'scaled ({0:0.4f}ifb)'.format(scaleFactor)]):
  ...
  print(printStr.format("GROUP", "VR0L", "VR1L"))
